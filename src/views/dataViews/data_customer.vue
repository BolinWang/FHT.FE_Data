 ...
 * @Last Modified time: 2018-06-06 18:02:00
 */
...
import { exportCustomerApi } from '@/api/dataReport'
export default {
  name: 'data_customer',
  components: {
    GridUnit
    ...
    handleExport() {
      ...
      exportCustomerApi(this.params).then(response => {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(['序号', ...tHeader], data, `租客数据${this.params.startDateStr} - ${this.params.endDateStr}`, `租客数据${this.params.startDateStr} - ${this.params.endDateStr}`)
