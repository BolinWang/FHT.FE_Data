 ...
 * @Last Modified time: 2018-06-07 10:52:44
 */
...
import { exportFhAccountApi } from '@/api/dataReport'
export default {
  name: 'data_fhAccount',
  components: {
    GridUnit
  ...
  data() {
    ...
    return {
      ...
      colModels: [
        ...
        {prop: 'fhtOnlineFee', label: '复恒公账支付金额', sortable: true},
        {prop: 'fhtOnlineFeeForBank', label: '复恒公账支付金额(银行卡)', sortable: true},
  ...
  methods: {
    ...
    handleExport() {
      ...
      exportFhAccountApi(this.params).then(response => {
        ...
        require.ensure([], () => {
          ...
          export_json_to_excel(['序号', ...tHeader], data, `复恒公账数据${this.params.startDateStr} - ${this.params.endDateStr}`, `复恒公账数据${this.params.startDateStr} - ${this.params.endDateStr}`)
